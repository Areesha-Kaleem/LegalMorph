...
from moj_laws_ksa import scrape_moj_laws_ksa
...
def refresh_metadata(db_name, collection_name):
    ...
    try:
        ...
        if last_metadata:
            ...
            print(f"🔍 Debug: Refreshed metadata for MOJ Laws KSA")
            return True
        else:
...
def initialize_session_state():
    ...
    if 'last_metadata' not in st.session_state:
        ...
        default_collection_name = st.session_state.get('metadata_collection_name', 'moj_laws_scrapping')
...
def main():
    st.set_page_config(
        page_title="KSA MOJ Laws Scraper",
        page_icon="⚖️",
        layout="wide",
    ...
    if ('scraping_thread' in st.session_state and 
        ...
        if 'scraping_status' in st.session_state and st.session_state.scraping_status == "running":
            ...
            refresh_metadata(st.session_state.get('metadata_db_name', 'LawGPT_Metadata_KSA'), 
                           st.session_state.get('metadata_collection_name', 'moj_laws_scrapping'))
    ...
    st.markdown('<h3 style="text-align: center; color: #888; margin-bottom: 2rem;">For MOJ Laws</h3>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        html_output_dir = st.text_input(
            ...
            value=st.session_state.get('html_output_dir', "D:\\LawGPT_data_pipeline\\data\\ksa\\raw\\moj_laws_ksa\\raw_html"),
        ...
        text_output_dir = st.text_input(
            ...
            value=st.session_state.get('text_output_dir', "D:\\LawGPT_data_pipeline\\data\\ksa\\raw\\moj_laws_ksa\\raw_text"),
            disabled=is_scraping,
        ...
        target_url = st.text_input(
            "Target URL:",
            value=st.session_state.get('target_url', "https://www.moj.gov.sa/English/Ministry/Departments/Mohammah/Pages/legalSystem.aspx?utm_source=chatgpt.co"),
            disabled=is_scraping,
            help="URL of the MOJ Laws page" + (" (Locked during scraping)" if is_scraping else "")
        )
        ...
        metadata_collection_name = st.text_input(
            ...
            value=st.session_state.get('metadata_collection_name', 'moj_laws_scrapping'),
            disabled=is_scraping,
    ...
    with col1:
        ...
        if 'scraping_status' in st.session_state:
            ...
            status_messages = {
                "running": {
                    "message": "Scraping MOJ Laws in progress...",
                    "class": "info-box"
                },
                "completed": {
...
def start_scraping(target_url, headless_mode, html_output_dir, text_output_dir, metadata_db_name, metadata_collection_name):
    ...
    try:
        ...
        print(f"Starting MOJ Laws KSA scraper...")
        ...
        def run_scraper():
            try:
                ...
                html_path, text_path = scrape_moj_laws_ksa(
