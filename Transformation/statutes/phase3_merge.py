...
def find_exact_match(base_filename, custom_files):
    """Find exact match between base and custom files"""
    base_name = os.path.splitext(base_filename)[0]

    for custom_file in custom_files:
        custom_name = os.path.splitext(custom_file)[0]
        if custom_name == base_name:
            return custom_file
    return None

def merge_statutes_json_gpt(base_dir, custom_dir, output_dir, issues_dir_base, issues_dir_custom, system_prompt, client, source_web):
    issue_count = 0
    raw_total = 0
    merge_statute_names = []
    merge_issue_statutes = []
    merge_total = 0
    processed = 0
...
"""}
                        ...
                        try:
                            ...
                            success = True
                            merge_statute_names.append(os.path.splitext(custom_file)[0])
                            merge_total += 1
                            break
                ...
                if not success:
                    issue_count += 1
                    merge_issue_statutes.append(custom_file)
                    ...
                    try:
                        ...
                        # Copy failed files into merge issues tree: .../merge/issues_statutes/<source>/{base|custom}
    ...
    return issue_count, merge_total, merge_statute_names, merge_issue_statutes, raw_total, processed


def merge_statutes_issue_resolver(base_dir, custom_dir, output_dir, system_prompt, client, source_web):
    count = 0
    custom_files = [f for f in os.listdir(custom_dir) if f.lower().endswith(".json")]
